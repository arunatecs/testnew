...
def fitbit_callback(request):
    ...
    if ecg_response.status_code == 200:
        ...
        # Generate time points with 250 interval
        #time_points = np.linspace(30, 7500, len(ecg_data))
        #time_points = np.linspace(30, 30 + len(ecg_data) / 250, len(ecg_data))
        # Calculate the time points array with units in seconds
        # Calculate the time points array with units in seconds
        num_data_points = len(ecg_data)
        duration_seconds = num_data_points / 250  # Total duration of the ECG data in seconds
        time_points = np.arange(30, 30 + duration_seconds, 1 / 250)

        # Plot ECG data against time points
        #plt.plot(time_points, ecg_data)
        #plt.xlim(30, 7500)
        # Adjust x-axis ticks
        
        
        # Adjust subplot margins to utilize full height
        ...
        length = len(ecg_data)
        print(length)
        x_ticks = np.arange(30, 30 + duration_seconds, 1 / 250) 
        ...
        #plt.xticks(np.arange(30, 7501, step=250), 
                   #labels=[str(i) for i in np.arange(30, 7501, step=250)])
        # plt.xticks(np.arange(30, 7501, step=250))
        # Set plot title
        plt.title('ECG Chart')            
        # Set x-axis label
