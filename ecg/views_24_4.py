...
def ecg_chart(request, ecg_id):
    ...
    db = client['django1']  # Replace 'your_database' with your actual database name
    collection = db['ecg_ecg']  # Replace 'your_collection' with your actual collection name

    # Retrieve a single document by _id using PyMongo
    document = collection.find_one({'_id': ObjectId(ecg_id)})
    #print(document)
    # Retrieve the Ecg object by _id
    #ecg_object = Ecg.objects.get(pk=ecg_id)
    #print(ecg_object)
    # Extract ECG data from the Ecg object
    if document:
