...
def ecg_list(request):
    ...
    ecg_readings = list(Ecg.objects.order_by('-timestamp')[:5].values())  # recent 5 records
...
def ecg_chart(request,ecg_id):
    ...
    print('hello')
    print(ecg_id)
    ecg_object_id = ObjectId(ecg_id)
    print(ecg_id)    
        # Retrieve ECG document from MongoDB using Djongo
    ecg_reading = Ecg.objects.filter(_id=ecg_object_id).first()


    print(ecg_reading)
    if ecg_reading:
        value = ecg_reading.value 
        ...
        return HttpResponse('ECG not found')
def home(request):
