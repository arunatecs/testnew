    ...
    if response.status_code == 200:
        ...
        #activity = data['activities'][0]
           ...
           # Extracting values for the chart
            zones = [zone['name'] for zone in heart_rate_zones]
            minutes = [zone['minutes'] for zone in heart_rate_zones]
            ...
            # Creating a single bar with stacked segments
            bars = ax.bar(start_time, minutes[0], color=colors[0], label=zones[0])

            # Adding additional bars on top to stack them
            for i in range(1, len(minutes)):
                bars = ax.bar(start_time, minutes[i], bottom=sum(minutes[:i]), color=colors[i], label=zones[i])

            # Adding values on the bars
            for i in range(len(minutes)):
                if minutes[i] > 0:
                    ax.text(
                        0,  # Since there's only one bar, x position is constant
                        sum(minutes[:i]) + minutes[i] / 2,
                        zones[i],
                        ha='center',
                        va='center',
                        color='white',
            ...
            ax.set_title(f'Heart Rate Zones during Outdoor Bike Activity on {start_time}', fontsize=14)
            ax.set_xlabel(f'Start Time: {start_time}', fontsize=12)
            ...
            plt.tight_layout()
            plt.show()
            image_path = f'acc_{idx}.png'  # Unique image path for each activity
...
def dashboard(request):
  ...
  # Setting y-axis limit to 50
    ...
    # all_minutes.append(minutes_for_activity)
