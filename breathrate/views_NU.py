...
def fitbit_callback(request):
    ...
    if response.status_code == 200:
        ...
        for idx, activity in enumerate(data):
            ...
            bars = ax.bar(idx, minutes[0], color=colors[0], label=zones[0])

            # Adding additional bars on top to stack them
            for i in range(1, len(minutes)):
                bars = ax.bar(idx, minutes[i], bottom=sum(minutes[:i]), color=colors[i], label=zones[i])

            # Adding values on the bars
            for i in range(len(minutes)):
                if minutes[i] > 0:
                    ax.text(
                        idx,  # Using idx as x position
